...
def svm_algorithm(data, attr, bias, lf, flag, t, margin):
	cum = 0.0
	prediction = []
...
def cross_validation_svm(lf, margin):
	cv_result = []
	for i in range(0,5,1):
		...
		for epoch in range(CV_EPOCH):
			weight, temp_bias, prediction, time = svm_algorithm(training_set, weight, bias, lf, TRAIN, time, margin)
		
		#clarify	
		weight, temp_bias, prediction, time = svm_algorithm(test_set, weight, temp_bias, lf, TEST, time, margin)
...
margin = [10, 1, 0.1, 0.01, 0.001, 0.0001]
...
def wrapper_svm():
	...
	for i in learning_factor:
		for j in margin:
			cv_out = cross_validation_svm(i, j)
	...
	print ("Hyper parameters: Learning rate %s , Regularization %s and its Accuracy %s" % (best_lf, best_margin, maximum))
	...
	for epoch in range(TEST_EPOCH):
		weight, temp_bias, prediction, time = svm_algorithm(train_set, weight, bias, best_lf, TRAIN, time, best_margin)

		w, tb, pr, time = svm_algorithm(dev_set, weight, temp_bias, best_lf, TEST, time, best_margin)
	...
	w2, b2, p2, t2 = svm_algorithm(test_set, b_weight, b_bias, best_lf, TEST, time, best_margin)
	...
	print ("Learning rate %s, Regularization %s and its test_set accuracy is %.4f" %(best_lf, best_margin, a))
